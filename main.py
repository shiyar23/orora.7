...
def handle_callback(call):
    ...
    # TP1-TP3
    if action.startswith('tp') and len(action) <= 3:
        idx = int(action[2]) - 1
        if 0 <= idx < 3 and not data.get(f'tp{idx+1}_done'):
            data[f'tp{idx+1}_done'] = True
            was_done = True
        elif idx >= 3 and action[2:].isdigit():
            tp_num = int(action[2:])
...
# تعديل SL
    ...
    if action.startswith('tp'):
        ...
        if tp_num.isdigit():
            tp_index = int(tp_num)
            tp_price = data['tp_prices'][tp_index-1] if tp_index <= 3 else data['extra_tps'][tp_index-4]
...
# معالج الرسائل النصية (لـ TP و SL)
...
def handle_user_input(message):
    ...
    current_msg_id = user_data[user_id].get('msg_id')
